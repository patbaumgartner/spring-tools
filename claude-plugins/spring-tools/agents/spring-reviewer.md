...
2. `getProjectDiagnostics` per project. For each Spring diagnostic code read the playbook `${CLAUDE_PLUGIN_ROOT}/explanations/<CODE>.md` and describe the fix precisely (file, range, what to change) instead of pasting the playbook.
