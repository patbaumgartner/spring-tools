...
You have encountered the Spring Boot diagnostic error code: `$code`.
The problem is located in the file: `$file`.
The text range of the problem is: `$range` (empty when the caller did not pass one).
...
`${CLAUDE_PLUGIN_ROOT}/explanations/$code.md`
...
2. Analyze the context of the user's project and the specific file (`$file`) to determine which of the suggested fixes is the most appropriate.
...
4. Once a solution is chosen (either by your analysis or the user's choice), apply the fix to the codebase, specifically targeting the file `$file`.
5. If a text range (`$range`) is provided, ensure your fix is applied only around that specific text range.
