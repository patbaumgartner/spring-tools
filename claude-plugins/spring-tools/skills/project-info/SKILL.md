...
4. When a project you expect is missing - e.g. it was just created or checked out and the list has not caught up - invoke `/spring-tools:refresh` and call `getProjectList` again. The server also watches the workspace, so new `pom.xml`/`build.gradle` files are normally discovered within a second by themselves.
5. For version support questions continue with `/spring-tools:spring-versions`; for validation problems with `/spring-tools:validate`.
